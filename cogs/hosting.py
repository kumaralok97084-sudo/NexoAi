...
from cogs.emojis import CROSS_NO, ONLINE_DOT
...
class HostingCog(commands.Cog):
    ...
    async def pinghost(self, interaction: discord.Interaction, host: str) -> None:
        ...
        try:
            ...
            await interaction.followup.send(f"{ONLINE_DOT}\n```\n{output}\n```")
        except (subprocess.TimeoutExpired, FileNotFoundError):
            await interaction.followup.send(f"{CROSS_NO} Ping to `{host}` timed out or failed.")
    ...
    async def dns_lookup(self, interaction: discord.Interaction, domain: str) -> None:
        ...
        try:
            ...
            lines = [f"{ONLINE_DOT} **{domain}** resolves to:"]
            for ip in ips[:10]:
                lines.append(f"`{ip}`")
            await interaction.followup.send("\n".join(lines))
        except socket.gaierror:
            await interaction.followup.send(f"{CROSS_NO} Could not resolve `{domain}`.")
    ...
    async def whois_lookup(self, interaction: discord.Interaction, domain: str) -> None:
        ...
        try:
            ...
            await interaction.followup.send(f"{CROSS_NO} WHOIS is not installed on this system.")
    ...
    async def ssl_check(self, interaction: discord.Interaction, domain: str, port: int = 443) -> None:
        ...
        try:
            ...
            embed = discord.Embed(title=f"{ONLINE_DOT} SSL Certificate: {domain}", color=discord.Color.green())
            ...
            await interaction.followup.send(f"{CROSS_NO} SSL check failed: `{exc}`")
    ...
    async def port_scan(self, interaction: discord.Interaction, host: str) -> None:
        ...
        for port in common_ports:
            try:
                ...
                pass
        if open_ports:
            await interaction.followup.send(f"{ONLINE_DOT} Open ports on `{host}`: `{' '.join(str(p) for p in open_ports)}`")
        else:
            await interaction.followup.send(f"{CROSS_NO} No common ports open on `{host}`.")
    ...
    async def trace_route(self, interaction: discord.Interaction, host: str) -> None:
        ...
        try:
            ...
            await interaction.followup.send(f"{CROSS_NO} Traceroute not available on this system.")
    ...
    async def http_check(self, interaction: discord.Interaction, url: str) -> None:
        ...
        try:
            async with httpx.AsyncClient(timeout=15) as client:
                ...
                embed = discord.Embed(title=f"{ONLINE_DOT} HTTP Check: {url}", color=discord.Color.green() if resp.is_success else discord.Color.red())
            ...
            await interaction.followup.send(f"{CROSS_NO} Failed to reach `{url}`: `{exc}`")
    ...
    async def http_headers(self, interaction: discord.Interaction, url: str) -> None:
        ...
        try:
            ...
            await interaction.followup.send(f"{ONLINE_DOT} **Headers for {url}**\n" + "\n".join(lines))
        except Exception as exc:
            await interaction.followup.send(f"{CROSS_NO} Failed: `{exc}`")
