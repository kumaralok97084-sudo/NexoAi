...
class UtilityCog(commands.Cog):
    ...
    async def role_menu(
        ...
        if role >= interaction.guild.me.top_role:
            await interaction.response.send_message("Role is above my highest role.", ephemeral=True)
            return
        try:
    ...
    async def remind(
        ...
        await interaction.response.send_message(f"⏰ Reminder set for {minutes} minute(s) (ID: {rid}).", ephemeral=True)
    ...
    async def add_note(self, interaction: discord.Interaction, title: str, content: str) -> None:
        ...
        await interaction.response.send_message(f"📝 Note saved (ID: {nid}).", ephemeral=True)
    ...
    async def list_notes(self, interaction: discord.Interaction) -> None:
        ...
        if not notes:
            await interaction.response.send_message("No notes saved.", ephemeral=True)
            return
    ...
    async def color_picker(self, interaction: discord.Interaction, hex_color: str) -> None:
        ...
        if len(hex_color) != 6:
            await interaction.response.send_message("Invalid hex color. Use format: FF00AA", ephemeral=True)
            return
        try:
    ...
    async def define(self, interaction: discord.Interaction, word: str) -> None:
        ...
        try:
            async with httpx.AsyncClient(timeout=10) as client:
                ...
                embed = discord.Embed(title=f"📖 {word}", color=discord.Color.blue())
        ...
        try:
            async with httpx.AsyncClient(timeout=10) as client:
                ...
                if resp.status_code == 200:
                    ...
                    await interaction.followup.send(f"Could not get weather for `{city}`.")
        except Exception as exc:
            await interaction.followup.send(f"Weather lookup failed: `{exc}`")

    @app_commands.command(name="timer", description="Start a simple countdown timer.")
    async def timer(self, interaction: discord.Interaction, seconds: app_commands.Range[int, 1, 3600]) -> None:
        await interaction.response.send_message(f"⏱️ Timer set for {seconds} seconds.", ephemeral=True)
        await asyncio.sleep(seconds)
        await interaction.channel.send(f"⏰ {interaction.user.mention} Your {seconds}-second timer is done!")
